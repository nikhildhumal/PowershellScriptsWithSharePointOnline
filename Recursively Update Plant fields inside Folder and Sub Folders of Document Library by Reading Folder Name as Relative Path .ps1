...
$SiteUrl = "https://gcpat.sharepoint.com/teams/Quality/GPC-Source-SBM"       
...
$allItems = @()
    ...
    $camlQuery.FolderServerRelativeUrl= "/teams/Quality/GPC-Source-SBM/Formulas/SBM"
 ## Executing the query
	...
	if($File.FileSystemObjectType -eq "File")
	{
		...
		{     
			...
			{							
					...
					Write-Host $($File["ID"])				
					"FileName: $($File["FileLeafRef"]), ID:$($File["ID"])" | Add-Content -Path 'E:\Logs Folder\SBMPlantFieldsFromFile.txt'
					break
			}
		}
...
Catch
{
    ...
    Write-Host ""
    "Error: $SPOConnectionException" | Add-Content -Path 'E:\Logs Folder\SBMPlantFieldsFromFile.txt'		
    Break
}
