...
$list = $lists.GetByTitle("TESTFormulas")
...
Write-Host $SearchArray.Length
		...
		For ($i=0; $i -lt $PlantArray.Length; $i++) 
		{ 
			    
			Write-host "Title: $PlantArray[$i] `r`n "
            $text = $PlantArray[$i]
	        $SearchArray[$i] | Add-Content -Path 'C:\Users\amit_dahotre\Desktop\Searcharray.txt'
            
		}
            #$PlantArray | Set-Content -Path 'C:\Users\amit_dahotre\Desktop\UpdateFileNameLogs.txt'	
            $PlantArray | Add-Content -Path 'C:\Users\amit_dahotre\Desktop\UpdateFileNameLogs.txt'
